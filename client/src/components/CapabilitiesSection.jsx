...
const capabilities = [
  {
    ...
    desc: 'Advanced AI analyzes your input, identifies the core concepts, and generates targeted search queries.',
  },
  {
    icon: Search,
...
export default function CapabilitiesSection() {
  return (
    <section className="py-16 sm:py-24 md:py-32 px-4 sm:px-6 relative border-t border-athena-border">
          ...
          {capabilities.map((cap, i) => {
            ...
            return (
              <motion.div
                ...
                className="border border-athena-border p-4 sm:p-5 md:p-6 hover:border-athena-purple/30 transition-all duration-300 group"
              >
                <div className="w-10 h-10 border border-athena-purple/20 bg-athena-purple/5 flex items-center justify-center mb-2 sm:mb-3 md:mb-4 group-hover:bg-athena-purple/10 transition-colors">
                  <Icon className="w-5 h-5 text-athena-purple" />
                </div>
                <h3 className="font-medium text-athena-offwhite mb-1 sm:mb-2 text-sm md:text-base">{cap.title}</h3>
                <p className="text-xs sm:text-sm text-athena-offwhite/40 leading-relaxed">{cap.desc}</p>
              </motion.div>
            );
          })}
        </div>
